        ...
        with st.container():
            ...
            if st.button("🚀 確認提交報單"):
                if not cust_id: st.error("請填寫老闆 ID")
                    ...
                    payload = {"date": datetime.now().strftime("%Y-%m-%d"), "slayer_id": st.session_state['user_id'], "rate_type": f"{tier}({int(st.session_state['user_rate']*100)}%)", "customer_id": cust_id, "item": f"{item_name} x{dur}", "price": total_price, "discount": disc, "slayer_cut": user_cut, "profit": total_price - user_cut}
