...
# 階級化定價邏輯 (僅針對計時項目)
def get_tier_price(item_name, base_price, tier):
    if tier == "魔王" and "計時" in item_name:
        if "台服-計時" in item_name: return 1200
        if "陸服-計時" in item_name: return 1500 # 舉例陸服魔王計時
        if "航天計時" in item_name: return 1500 # 航天魔王
    elif tier == "巔峰" and "計時" in item_name:
        if "台服-計時" in item_name: return 1500
        if "航天計時" in item_name: return 1800 # 巔峰航天
    return base_price
...
if st.session_state['user_type'] is None:
    ...
    if login_type == "管理員 (Admin)":
        ...
        if st.button("打手登入"):
            ...
            if not staff_df.empty and player_id.strip() in staff_df['打手ID'].values:
                ...
                tier = player_info[tier_col[0]] if tier_col else "普通"
    ...
    if user_type == "slayer":
        ...
        with st.container():
            ...
            item_name = r2c2.selectbox("選擇護航項目", list(ITEMS_DATA.keys()))
            ...
            # 階級定價計算
            base_p = ITEMS_DATA[item_name]
            unit_p = get_tier_price(item_name, base_p, st.session_state.get('user_tier', '普通'))
            total_price = (unit_p * dur) - disc
            ...
            if st.button("🚀 確認提交報單"):
                if not cust_id: st.error("請填寫老闆 ID")
                    ...
                    payload = {"date": datetime.now().strftime("%Y-%m-%d"), "slayer_id": st.session_state['user_id'], "rate_type": f"{int(st.session_state['user_rate']*100)}%", "customer_id": cust_id, "item": f"{item_name} (x{dur})", "price": total_price, "discount": disc, "slayer_cut": user_cut, "profit": total_price - user_cut}
        ...
        st.divider(); st.subheader("📅 我的報單歷史紀錄")
        ...
        if not df.empty:
            ...
            m1.metric("總營收", f"${int(pd.to_numeric(df['單價'], errors='coerce').sum())}")
            m2.metric("總利潤", f"${int(pd.to_numeric(df['公司利潤'], errors='coerce').sum())}")
            m3.metric("訂單總數", len(df))
